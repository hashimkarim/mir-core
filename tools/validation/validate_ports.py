...
def command_plan(output, scratch, legacy_run=None, *, precise_android=False):
    ...
    add('native', 'runtime-python-build', [py, '-m', 'maturin', 'build', '--release', '--locked',
        '--out', scratch/'python-wheels'], desktop/'native-runtime')
    add('native', 'runtime-python-unpack', [py, '-c',
        'import pathlib,sys,zipfile; wheels=list(pathlib.Path(sys.argv[1]).glob("mir_native_runtime-*.whl")); '
        'assert len(wheels)==1, wheels; zipfile.ZipFile(wheels[0]).extractall(sys.argv[2])',
        scratch/'python-wheels', scratch/'python'])
    ...
    for example, fixture in [('check_replay', 'port_reference.json')]:
    ...
    add('firmware', 'pp-firmware', ['pio', 'run', '-e', 'seeed_xiao_esp32s3', '-e', 'seeed_xiao_esp32s3_plus'], pp)
...
def main():
    ...
    environment.update(PYTHONPATH=os.pathsep.join([str(scratch/'python'), *(str(ROOT/name) for name in
